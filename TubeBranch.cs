...
namespace Proyek_UTS_Grafkom
{
    class TubeBranch
    {
        ...
        public void createbezierVertices(float radiusX,float radiusY,float radiusZ)
        {
            Vector3 temp_vector;
            int i = 0;
            ...
            for (float t = 0; t < 1.0f; t += 0.01f)
            {
                ...
                for (float u = -_pi; u <= _pi; u += _pi / 60)
                {
                    for (float v = -_pi / 2; v < _pi / 2; v += _pi / 60)
                    {
                        temp_vector.X = p.X + (float)Math.Cos(u) * radiusX; // x
                        temp_vector.Y = p.Y + (float)Math.Sin(u) * radiusY; // y
                        temp_vector.Z = p.Z + v * radiusZ; // z
                        vertices.Add(temp_vector);
                    }
                }
                vertices.Add(p);
